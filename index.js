...
//    @param (10) keep denotes the number characters to keep when transitionong out
...
//    * @return null
  ...
  $.fn.tickerText = function (
    ...
    let maxInDelay = Math.floor(
      (seconds * inTransPercent * 1000) /
        contents[count % contents.length].textContent.length
    );
      ...
      timelineTimerAdd = setTimeout(
        function () {
          ...
          if (current < textArray.length && count >= 0) {
            ...
            // parent
            if (
              textArraySerialised[0][
                current - childCharCounted + 2 * childCounted
              ] !== "/" &&
              currentChild === 0
            ) {
              elem.append(textArray[current]);
            ...
            if (
              ...
              if (
                textArraySerialised[0][
                  current + 1 - childCharCounted + 2 * childCounted
                ] == childCount
              ) {
                if (currentChild === 0) {
                  currentChild = childCount;
                  childLength = $(textArraySerialised[childCount]).text()
                    .length;
                ...
                $(elem.children()[childCounted]).html(function (
                  index,
                  oldHTML
                ) {
                  return oldHTML;
    ...
    let maxOutDelay = Math.floor(
      (1000 * (seconds * outTransPercent)) / elem.text().length
    );

    // subtract function
    function timeoutSubtract() {
      timelineTimerSubract = setTimeout(
        function () {
          if (current >= 0) {
            //child
            if (
              textArraySerialised[0][
                current - childCharCounted + 2 * childCounted - 2
              ] === "/"
            ) {
              ...
              if (
                textArraySerialised[0][
                  current - childCharCounted + 2 * childCounted - 1
                ] == childCounted
              ) {
                if (currentChild === 0) {
                  currentChild = childCounted;
                  childLength = $(textArraySerialised[childCounted]).text()
                    .length;
                }
            ...
            if (
              ...
              let shortenedString = elem
                .html()
                .substring(0, elem.html().length - 1);
      ...
      timelineTimerNoop = setTimeout(function () {
        if (!exit && !pause && vis()) {
          if (dev) {
            dyy = performance.now();
            console.log(
              "timeline Noop end for iterations dxx - dyy, count final",
              count,
              dyy - dxx,
              "microseconds"
            );
        ...
        if (!exit && (pause || !vis())) {
          if (dev) {
            dyy = performance.now();
            console.log(
              "timeline Noop for iterations dxx - dyy, count final",
              count,
              dyy - dxx,
              "microseconds"
            );
    ...
    function part4() {
      masterTimelineTimerSubtractTidy = setTimeout(function () {
        ...
        if (secondsout && count === 0) {
          seconds = secondsout;
          // update maxInDelay
          maxInDelay = Math.floor(
            (seconds * inTransPercent * 1000) /
              contents[count % contents.length].textContent.length
          );
          // update maxutDelay
          maxOutDelay = Math.floor(
            (1000 * (seconds * outTransPercent)) / text.length
          );
        }
        ...
        if (!exit && (!pause || vis())) {
          ...
          textArraySerialised =
            scontent.content[[count % scontent.content.length]];
